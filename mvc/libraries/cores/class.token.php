...
class Token extends Application{
  private $PDODB;
  private $config;
  ...
  /*
	...
	*/	
	private function aesEncrypt($value){
		if (strlen($value)>1){
			$iv_size = mcrypt_get_iv_size(\MCRYPT_RIJNDAEL_256, \MCRYPT_MODE_CBC);
			$iv = mcrypt_create_iv($iv_size,\MCRYPT_RAND);
			return base64_encode($iv.mcrypt_encrypt(\MCRYPT_RIJNDAEL_256, $this->config['pkey'], $value, \MCRYPT_MODE_CBC, $iv));
		} 
		return '';
	}
	...
	*/
	private function aesDecrypt($value){
		if (strlen($value)>1){
			...
			return trim(mcrypt_decrypt(\MCRYPT_RIJNDAEL_256, $this->config['pkey'], $value, \MCRYPT_MODE_CBC, $iv));
		}
		return '';
	}
